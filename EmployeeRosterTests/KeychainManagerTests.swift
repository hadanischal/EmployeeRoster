...
class KeychainManagerTests: QuickSpec {

    override func spec() {
        let mockKey = "mock.nischal.keychain.key"

        describe("KeychainManager") {
            var testModel: KeychainManager!
            var testScheduler: TestScheduler!

            beforeEach {
                testScheduler = TestScheduler(initialClock: 0)
                testModel = KeychainManager(withKeyChainServiceKey: mockKey, andScheduler: MainScheduler.instance)
            }
